...
- **2026-02-10 반영:** (1) **MigrationPage 파일명 인코딩:** `toSafeStoragePath(originalName, prefix)` — Mac/한글 파일명 MIME 오류 방지. timestamp+영문 확장자(pdf/jpg/png)로 저장. 견적·원가 업로드 모두 적용. (2) **저장 = 확정:** 견적 임시저장(handleEstimateSaveDraft) 시 insert/update 모두 `approved_at` 자동 설정. (3) **6개월 견적 통계:** ConsultationManagement 견적 관리 탭. estimatesLast6Months(created_at 6개월 이내, is_visible=true). 최대·최소·**실제 중간값**(정렬 후 중앙에 가까운 견적) + 각 estimate_id 매핑. (4) **원가 연동:** getVendorPriceRecommendation으로 최근 견적 품목 원가 합산(vendor_price_book → products). (5) **견적 이력 UI:** [최대|중간|최소|원가] 4가지 지표. 최대/중간/최소 클릭 → setPrintEstimateId로 견적서 팝업. 툴팁 "해당 견적서 보기", cursor-pointer.
