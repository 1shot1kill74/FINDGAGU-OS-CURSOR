...
export const OAUTH_ROW_ID = 'findgagu'

export type YoutubeAnalyticsOauthRow = {
  id: string
  channel_id: string
...
export async function fetchMineChannel(accessToken: string) {
...
export async function loadOauthRow(): Promise<YoutubeAnalyticsOauthRow | null> {
  const supabase = createServiceSupabase()
...
export async function markNeedsReconnect(message: string) {
  const supabase = createServiceSupabase()
  await supabase
    .from('youtube_analytics_oauth')
    .update({
