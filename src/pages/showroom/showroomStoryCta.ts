...
export function buildShowroomStoryBackHref(concern: string | null | undefined): string {
  ...
  if (!validConcern) {
    return '/public/showroom#showroom-concern-result-anchor'
  }
  return `/public/showroom?${new URLSearchParams({ concern: validConcern }).toString()}#showroom-concern-result-anchor`
}
