...
type CaseDraftSeedRow = Omit<CaseDraftState, 'cardNewsSlides'>

type FrameTemplateEditorState = {
  id: string
  label: string
  body: string
}

const SLIDE_KEY_OPTIONS: { value: ShowroomCaseCardNewsSlide['key']; label: string }[] = [
  { value: 'hook', label: '훅' },
...
function buildStudioContentSeed(row: CaseDraftSeedRow): ShowroomCaseContentSeed {
...
function buildTemplatedCaseSeed(row: CaseDraftState): ShowroomCaseContentSeed {
...
function buildTemplatedStudioSlides(row: CaseDraftState): StudioCardNewsSlide[] {
...
function studioSlidesFromResponse(
  response: unknown,
...
function deriveStudioSeedFromSlides(row: CaseDraftState): ShowroomCaseContentSeed {
  if (
    row.problemCode
    || row.solutionCode
...
function studioRowToCardPackage(row: CaseDraftState): ShowroomCaseCardNewsPackage {
...
function getGenerationStatusLabel(status: CaseDraftState['cardNewsGeneration']['status']) {
  switch (status) {
    case 'processing':
      return '생성 중'
    case 'completed':
...
function getGenerationStatusTone(status: CaseDraftState['cardNewsGeneration']['status']) {
  switch (status) {
    case 'processing':
...
function formatGenerationTimestamp(value: string | null) {
  if (!value) return ''
...
function buildPublicCardNewsPath(siteKey: string) {
  return `/public/showroom/cardnews/${encodeURIComponent(siteKey)}`
}

const PROBLEM_FRAME_OPTIONS = [
...
function groupBeforeAfter(assets: ShowroomImageAsset[]) {
