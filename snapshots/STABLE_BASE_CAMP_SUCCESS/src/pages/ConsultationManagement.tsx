import React, { useState, useEffect, useMemo, useRef, Component, type ErrorInfo } from 'react'
import { ChevronLeft } from 'lucide-react'
import { Link } from 'react-router-dom'
import { Plus, Calculator, RefreshCw, Zap, Phone, Copy, User, Images, Check, MessageCircle, Pencil, Loader2, Search, FileText, CheckCircle, Ruler } from 'lucide-react'
...
type DateRangeKey = 'all' | '1m' | '3m' | '6m' | '1y'
...
/** 인입채널 배지 — source 있으면 항상 표시(핵심 데이터 보존). 오픈마켓=마켓배지, 그 외=중립 배지 */
function SourceChannelBadge({ source }: { source?: string }) {
  if (!source || !source.trim()) return null
  if (isMarketSource(source)) return <MarketSourceBadge source={source} />
  return (
    <span className="shrink-0 rounded px-1.5 py-0.5 text-[11px] font-medium bg-slate-200/80 text-slate-700 dark:bg-slate-600/50 dark:text-slate-300" title={`인입: ${source}`}>
      {source}
    </span>
  )
}
...
/** 마지막 활동 1건 (상담 히스토리 요약·알림 점·경과 시간·활동 아이콘용) */
...
function ConsultationListItem({
  ...
  const isActive24h = lastCreatedAt > 0 && Date.now() - lastCreatedAt < 24 * 60 * 60 * 1000

  return (
    <button
      type="button"
      onClick={onSelect}
      className={`relative w-full text-left rounded-md border transition-colors flex flex-col gap-0.5 px-2 py-1.5 min-h-0 ${isSelected ? 'bg-primary/10 border-primary' : 'bg-card border-border hover:bg-muted/50'}`}
    >
      {/* 활성 상태 알림 점: 마지막 활동 24시간 이내일 때 우측 상단 */}
      {isActive24h && (
        <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-blue-500 shrink-0" title="최근 24시간 내 활동" aria-hidden />
      )}
      {/* 1행: 좌측 [고객구분 배지] [디스플레이네임 Bold] [인입채널 배지] [골든타임 배지] · 우측 진행상태/AS/편집/복사 (gap-2, 겹치지 않게 일렬) */}
      <div className="flex flex-row items-center justify-between gap-2 min-h-[20px]">
        <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden flex-wrap">
          <CustomerTierBadge tier={item.customerTier} />
          <span className="font-bold text-foreground text-[15px] leading-tight truncate shrink-0" title={item.displayName}>
            {item.displayName}
          </span>
      ...
      {/* 2행: [주문번호] 지역 | 업종 | 전화번호 | 인입 | 요청 · 우측 고정폭 구글챗 버튼(3단계) */}
      <div className="flex items-center gap-1.5 text-[12px] min-h-[18px] flex-wrap">
        ...
        <span className="text-border">|</span>
        <span className="text-muted-foreground">{item.requiredDate ? `요청: ${item.requiredDate}` : '요청: —'}</span>
      ...
      {/* 3행: 요청사항(페인포인트) — 연한 배경, 슬림 */}
      <div className="min-h-[18px]">
        <p className="text-[13px] text-foreground bg-muted/60 dark:bg-muted/50 rounded px-2 py-1 w-full min-w-0 break-words line-clamp-2">
          {painText}
        </p>
      </div>
      {/* 카드 하단: 마지막 활동 경과 시간 (우측 구석) */}
      {lastMessage?.created_at && (
        <div className="flex justify-end min-h-[14px]">
...
export default function ConsultationManagement() {
  ...
  const tabCounts = useMemo(() => {
    ...
    const cut6m = subMonths(now, 6).getTime()
    const cut1y = subMonths(now, 12).getTime()
  ...
  const handleStageChange = async (leadId: string, stage: ConsultationStage) => {
    ...
    try {
      ...
      await insertSystemLog(supabase, {
        ...
        actor_name: actor,
        detail: `상담 단계를 ${prevStage} → ${stage}(으)로 변경함`,
  ...
  /** 견적서 승인 시 estimates 테이블 저장 + metadata.estimate_history 동기화 + 시스템 로그 */
  ...
  return (
    <div className="flex min-h-screen bg-background">
      <main className="flex-1 min-w-0 flex flex-col p-6">
            ...
            <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[480px] max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmitConsultation} className="space-y-4">
                  ...
                  {/* 6. 인입채널(Lead Source) — DB metadata.source에 저장, 절대 삭제 금지 */}
                  <div>
        ...
        <Dialog open={!!editModalLeadId} onOpenChange={(open) => !open && setEditModalLeadId(null)}>
          <DialogContent className="sm:max-w-[440px] max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid gap-3 pt-1">
              ...
              {/* 인입채널(Source) — 신규 폼과 동일 옵션, DB metadata.source 바인딩/업데이트, 삭제 금지 */}
              <div>
        ...
        {/* 좌측: 검색·기간·탭·리스트 | 우측: 히스토리 패널 — PC 고밀도 / 모바일: 선택 시 목록 숨김 */}
        <div className="flex gap-4 flex-1 min-h-0">
          <div className={`flex-1 min-w-0 flex flex-col gap-2 ${isMobile && selectedLead ? 'hidden' : ''}`}>
            {/* 검색 + 기간 필터 — 상단 고정 */}
          ...
          {/* 우측: 상담 상세 패널 — [상담 히스토리 | 실측 자료 | 견적 관리] 탭 / 모바일: 풀폭 + 목록으로 버튼 */}
          <aside
            className={`shrink-0 flex flex-col border border-border rounded-xl bg-card overflow-hidden transition-[width,opacity] duration-200 ${
              selectedLeadData
                ? (isMobile ? 'w-full opacity-100' : 'w-full sm:w-[400px] md:w-[420px] opacity-100')
                : 'w-0 opacity-0 pointer-events-none overflow-hidden border-0'
            }`}
          >
            {selectedLeadData && (
