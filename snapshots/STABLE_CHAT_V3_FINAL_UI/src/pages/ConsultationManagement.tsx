...
function ConsultationListItem({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`relative w-full text-left rounded-md border transition-colors flex flex-col gap-1 px-2 py-1.5 min-h-0 ${isSelected ? 'bg-primary/10 border-primary' : 'bg-card border-border hover:bg-muted/50'}`}
    >
      ...
      {/* 1행: 제목(디스플레이네임) 강조 + 배지들 · 우측 진행/AS/편집/복사 */}
      <div className="flex flex-row items-center justify-between gap-1.5 min-h-[20px]">
        <div className="flex items-center gap-1.5 min-w-0 flex-1 overflow-hidden flex-wrap">
          ...
          {item.isGoldenTime && (
            <span className="inline-flex items-center gap-0.5 rounded-full bg-amber-500/20 text-amber-700 dark:text-amber-400 px-1 py-0.5 text-[10px] font-semibold shrink-0">
        ...
        <div className="flex items-center gap-0.5 shrink-0" onClick={(e) => e.stopPropagation()}>
          ...
          <button
            ...
            className={`px-1 py-0.5 rounded text-[10px] font-medium shrink-0 ${(item.asRequested || item.status === 'AS_WAITING') ? 'bg-red-500/20 text-red-700 dark:text-red-400' : 'bg-muted text-muted-foreground hover:bg-muted/80'}`}
      ...
      {/* 3행: 인입/요청/금액(작은 글씨) + 구글챗 버튼 */}
      <div className="flex items-center justify-between gap-1 min-h-[16px]">
        <div className="flex items-center gap-1 text-[10px] text-muted-foreground min-w-0 truncate">
          {item.inboundDate && <span>인입 {item.inboundDate}</span>}
          {item.inboundDate && <span className="text-border">·</span>}
...
export default function ConsultationManagement() {
  ...
  /** 상담 카드 선택 시 선택 상태 반영 + last_viewed_at 갱신(읽음 처리) */
