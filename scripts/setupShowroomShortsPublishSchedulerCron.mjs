 ...
 *
 * 사용:
 *   node scripts/setupShowroomShortsPublishSchedulerCron.mjs
 *   node scripts/setupShowroomShortsPublishSchedulerCron.mjs --secret=existing_secret
 */
...
const CRON_JOB_NAME = 'showroom-shorts-publish-scheduler'
const CRON_EXPR = '* * * * *'
...
function main() {
  ...
  console.log('Edge Function 시크릿 SHOWROOM_SHORTS_PUBLISH_CRON_SECRET 등록…')
  execSync(`npx supabase secrets set SHOWROOM_SHORTS_PUBLISH_CRON_SECRET=${cronSecret}`, {
    stdio: 'inherit',
    cwd: process.cwd(),
  })
...
select cron.schedule(
  ...
  select net.http_post(
    url := '${PROJECT_URL}/functions/v1/showroom-shorts-publish-scheduler',
...
`
  ...
  const sqlPath = resolve(process.cwd(), '.tmp_shorts_publish_scheduler_cron.sql')
  ...
  console.log(`  - 대상: ${PROJECT_URL}/functions/v1/showroom-shorts-publish-scheduler`)
  console.log(
    '  - Edge 배포: npx supabase functions deploy showroom-shorts-publish-scheduler --project-ref sxxnshvidfwuemgbyuqz',
  )
