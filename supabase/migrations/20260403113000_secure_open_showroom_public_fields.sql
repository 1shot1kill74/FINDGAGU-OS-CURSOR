...
as $$
  ...
  select coalesce(
    public_display_name,
    nullif(regexp_replace(external_display_name, '\s+\d{4}$', ''), ''),
    nullif(trim(concat_ws(' ', month_label, location_label, coalesce(business_label, '기타'))), ''),
    '시공 사례'
  )
  from normalized;
$$;

comment on function public.open_showroom_display_name(jsonb, text, text, timestamptz) is
'오픈쇼룸에 노출할 안전한 현장 표시명. metadata 전체를 노출하지 않고 공개용 표시명만 만든다.';

create or replace function public.open_showroom_group_key(
  p_metadata jsonb,
  p_site_name text,
  p_asset_id uuid
)
...
as $$
  select coalesce(
    case
      when nullif(btrim(coalesce(p_metadata ->> 'space_id', '')), '') is not null
        then 'space:' || md5(btrim(p_metadata ->> 'space_id'))
      else null
    end,
    case
      when nullif(btrim(coalesce(p_metadata ->> 'before_after_group_id', '')), '') is not null
        then 'before-after:' || md5(btrim(p_metadata ->> 'before_after_group_id'))
      else null
    end,
    case
      when nullif(btrim(coalesce(p_metadata ->> 'canonical_site_name', '')), '') is not null
        then 'site:' || md5(btrim(p_metadata ->> 'canonical_site_name'))
      else null
    end,
    case
      when nullif(btrim(coalesce(p_site_name, '')), '') is not null
        then 'site:' || md5(btrim(p_site_name))
      else null
    end,
    'site:' || p_asset_id::text
  );
$$;

comment on function public.open_showroom_group_key(jsonb, text, uuid) is
'오픈쇼룸 카드/갤러리 그룹핑용 공개 키. 원본 현장명, space_id, before_after_group_id를 해시로 감춘다.';

grant execute on function public.open_showroom_display_name(jsonb, text, text, timestamptz) to anon, authenticated;
grant execute on function public.open_showroom_group_key(jsonb, text, uuid) to anon, authenticated;
...
as $$
  select
    ...
    ia.cloudinary_url,
    ia.thumbnail_url,
    public.open_showroom_display_name(coalesce(ia.metadata, '{}'::jsonb), ia.location, ia.business_type, ia.created_at) as site_name,
    ia.location,
    ia.business_type,
    ia.color_name,
    ia.product_name,
...
as $fn$
  ...
  select
    ...
    b.before_after_role
  from base b
  join site_order so on so.internal_group_key = b.internal_group_key
  where include_all
