-- 공개 쇼룸 RPC에 내부 현장명(source_site_name)을 함께 반환한다.
-- site_name 컬럼은 기존처럼 공개 표시명을 유지해 URL/카드 호환을 지키되,
-- 클라이언트는 source_site_name으로 승인 블로그 프로필과 매칭한다.
...
as $fn$
  with base as (
    select
      ...
      nullif(btrim(ia.site_name), '') as source_site_name,
      ia.location,
      ia.business_type,
      ia.color_name,
      ia.product_name,
