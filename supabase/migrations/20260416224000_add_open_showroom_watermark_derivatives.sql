alter table public.image_assets
  add column if not exists public_watermarked_url text,
  add column if not exists public_watermarked_thumbnail_url text,
  add column if not exists public_watermark_status text,
  add column if not exists public_watermark_version integer,
  add column if not exists public_watermark_updated_at timestamptz;

update public.image_assets
set
  public_watermark_status = coalesce(
    public_watermark_status,
    case
      when coalesce(is_consultation, false) = true
        and coalesce(category, '') not in ('purchase_order', 'floor_plan')
        and nullif(btrim(cloudinary_url), '') is not null
        then 'pending'
      else 'skipped'
    end
  ),
  public_watermark_version = coalesce(public_watermark_version, 1)
where public_watermark_status is null
   or public_watermark_version is null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'image_assets_public_watermark_status_check'
  ) then
    alter table public.image_assets
      add constraint image_assets_public_watermark_status_check
      check (public_watermark_status in ('pending', 'ready', 'failed', 'skipped'));
  end if;
end
$$;
...
as $fn$
  with base as (
    select
      ia.id,
      case
        when ia.public_watermark_status = 'ready'
          and nullif(btrim(ia.public_watermarked_url), '') is not null
          then ia.public_watermarked_url
        else coalesce(
          regexp_replace(ia.cloudinary_url, '/upload/', '/upload/f_auto,q_auto,c_limit,w_1600/', 1, 1),
          ia.thumbnail_url,
          ia.cloudinary_url
        )
      end as cloudinary_url,
      case
        when ia.public_watermark_status = 'ready'
          and nullif(btrim(ia.public_watermarked_thumbnail_url), '') is not null
          then ia.public_watermarked_thumbnail_url
        else coalesce(
          ia.thumbnail_url,
      ...
      ia.site_name,
      coalesce(ia.public_watermark_status, 'pending') as public_watermark_status
